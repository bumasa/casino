...
if ($send == "1") {
$send="";
mysql_query("update g_set_new set g_bank=$g_bank where g_name='chukcha'");
mysql_query("update g_set_new set g_proc=$g_proc where g_name='chukcha'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}

if ($send == "2") {
$send="";
mysql_query("update g_set_new set g_rezim=$g_rezim where g_name='chukcha'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
mysql_query("update g_set_new set g_shansbon='$g_shansbon' where g_name='chukcha'");
mysql_query("update g_set_new set g_shanswin='$g_shanswin' where g_name='chukcha'");
mysql_query("update g_set_new set g_rezerv='$g_shansdouble' where g_name='chukcha'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
<html>
<head>
<title>Настройка игры chukcha</title>
...
<center><b>Настройка игры Chukcha</b><br><a href=../../index.php> << Вернуться в админку </a><br><br></center>
...
<TR><TD align="left">Бонус 2 : </td><td>1 из <INPUT maxLength=20 size=10 name=g_shansbon2 value="<?echo $g_shansbon2;?>" style=" border: 1px solid rgb(0,0,0)"> min 2 </TD></TR>
<TR><TD align="left">Бонус 3 (в бонусе 2): </td><td>1 из <INPUT maxLength=20 size=10 name=g_shansbon3 value="<?echo $g_shansbon3;?>" style=" border: 1px solid rgb(0,0,0)"> min 2 </TD></TR>
...
list($betall) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='chukcha' "));
list($winall) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='chukcha' or game='chukcha_bonus' or game='chukcha_double' "));
$colbet=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='chukcha' "));

list($betall2) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='chukcha' && data='$dropdate' "));
list($winall2) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='chukcha' && data='$dropdate' or game='chukcha_bonus' && data='$dropdate' or game='chukcha_double' && data='$dropdate'"));
$colbet2=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='chukcha' && data='$dropdate' "));
$dohod=$betall-$winall;
...
<center><font color="green" size=2>© 2008 AzartSoft</font><br/>
