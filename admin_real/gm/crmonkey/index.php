...
if ($send == "1") {
$send="";
mysql_query("update g_set_new_cr set g_bank=$g_bank where g_name='crmonkey'");
mysql_query("update g_set_new_cr set g_proc=$g_proc where g_name='crmonkey'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}

if ($send == "2") {
$send="";
mysql_query("update g_set_new_cr set g_rezim=$g_rezim where g_name='crmonkey'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
mysql_query("update g_set_new_cr set g_shansbon=$g_shansbon where g_name='crmonkey'");
mysql_query("update g_set_new_cr set g_shanswin='$g_shanswin' where g_name='crmonkey'");
mysql_query("update g_set_new_cr set g_rezerv='$g_shansdouble' where g_name='crmonkey'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
if ($send == "4") {
$send="";
mysql_query("update g_set_new_cr set g_cask=$g_cask where g_name='crmonkey'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
<html>
<head>
<title>Настройка игры crmonkey</title>
...
<center><b>Настройка игры crmonkey</b><br><a href=../../index.php> << Вернуться в админку </a><br><br></center>
...
<td>
<FORM name="blabla1" action=index.php method=post>
<TR><TD align="center" valign="center"><h4><font color=green>Каска</font></h4></td><td>&nbsp;</td></tr>
<TR><TD align="right">При ставке в </td><td><INPUT maxLength=20 size=10 name=g_cask value="<?echo $g_cask;?>" style=" border: 1px solid rgb(0,0,0)"> </TD></TR>

<TR><TD align="right"><INPUT type=hidden value=4 name=send><INPUT type=submit value="Сохранить"></TD><td>&nbsp;</td></TR>
</FORM>
</td>
</table>
...
list($betall) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='crmonkey' "));
list($winall) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='crmonkey' or game='crmonkey_bonus' or game='crmonkey_double' "));
$colbet=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='crmonkey' "));

list($betall2) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='crmonkey' && data='$dropdate' "));
list($winall2) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='crmonkey' && data='$dropdate' or game='crmonkey_bonus' && data='$dropdate' or game='crmonkey_double' && data='$dropdate'"));
$colbet2=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='crmonkey' && data='$dropdate' "));
$dohod=$betall-$winall;
