...
if ($send == "1") {
$send="";
mysql_query("update g_set_new set g_bank=$g_bank where g_name='fairy_land'");
mysql_query("update g_set_new set g_proc=$g_proc where g_name='fairy_land'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}

if ($send == "2") {
$send="";
mysql_query("update g_set_new set g_rezim=$g_rezim where g_name='fairy_land'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
mysql_query("update g_set_new set g_shansbon=$g_shansbon where g_name='fairy_land'");
mysql_query("update g_set_new set g_shanswin='$g_shanswin' where g_name='fairy_land'");
mysql_query("update g_set_new set g_rezerv='$g_shansdouble' where g_name='fairy_land'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}



$row_bon=mysql_fetch_array(mysql_query("select * from g_set_new where g_name='fairy_land'"));
...
<html>
<head>
<title>Настройка игры fairy_land</title>
...
<center><b>Настройка игры Fairy Land 2</b><br><a href=../../index.php> << Вернуться в админку </a><br><br></center>
...
list($betall) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='fairy_land' "));
list($winall) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='fairy_land' or game='fairy_land_bonus' or game='fairy_land_double' "));
$colbet=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='fairy_land' "));

list($betall2) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='fairy_land' && data='$dropdate' "));
list($winall2) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='fairy_land' && data='$dropdate' or game='fairy_land_bonus' && data='$dropdate' or game='fairy_land_double' && data='$dropdate'"));
$colbet2=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='fairy_land' && data='$dropdate' "));
$dohod=$betall-$winall;
