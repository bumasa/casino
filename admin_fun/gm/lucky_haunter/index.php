...
if ($send == "1") {
$send="";
mysql_query("update g_set_new set g_bank=$g_bank where g_name='lucky_haunter'");
mysql_query("update g_set_new set g_proc=$g_proc where g_name='lucky_haunter'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}

if ($send == "2") {
$send="";
mysql_query("update g_set_new set g_rezim=$g_rezim where g_name='lucky_haunter'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
mysql_query("update g_set_new set g_shansbon='$g_shansbon' where g_name='lucky_haunter'");
mysql_query("update g_set_new set g_shanswin='$g_shanswin' where g_name='lucky_haunter'");
mysql_query("update g_set_new set g_rezerv='$g_shansdouble' where g_name='lucky_haunter'");
echo "<script> alert('Настройки сохранены!'); document.location.href='index.php';</script>";
}
...
<html>
<head>
<title>Настройка игры lucky_haunter</title>
...
<center><b>Настройка игры fruit lucky_haunter</b><br><a href=../../index.php> << Вернуться в админку </a><br><br></center>
...
list($betall) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='lucky_haunter' "));
list($winall) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='lucky_haunter' or game='lucky_haunter_bonus' or game='lucky_haunter_double' "));
$colbet=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='lucky_haunter' "));

list($betall2) = mysql_fetch_row(mysql_query("SELECT SUM(stav) FROM stat_game WHERE game='lucky_haunter' && data='$dropdate' "));
list($winall2) = mysql_fetch_row(mysql_query("SELECT SUM(win) FROM stat_game WHERE game='lucky_haunter' && data='$dropdate' or game='lucky_haunter_bonus' && data='$dropdate' or game='lucky_haunter_double' && data='$dropdate'"));
$colbet2=mysql_num_rows(mysql_query("SELECT * FROM stat_game WHERE game='lucky_haunter' && data='$dropdate' "));
$dohod=$betall-$winall;
...
<center><font color="blue" size=2>© 2009 AzartSoft</font><br/>
