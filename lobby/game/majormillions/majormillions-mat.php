...
if(!isset($_SESSION['l'])){ //защита от входа без авторизации
...
function winlimit( )
{
    ${ "bvbvbv" } = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='majormillions'" ) );
...
if ( $freenumspin == 0 )
{
    ...
    $rowb9 = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='majormillions'" ) );
    $proc4 = $rowb9['g_proc'];
    $allbet23 = $allbet / 100 * $proc4;

    mysql_query( "update g_set_new set g_bank=g_bank+$allbet23 where g_name='majormillions'" );
    $row_bon = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='majormillions'" ) );
    ...
    $psym[1] = array( 0, 0, 0, 10, 50, 100 ); // SCATER
    $psym[2] = array( 0, 0, 0, 0, 0, 0 );  // WILD
    $psym[3] = array( 0, 0, 5, 50, 500, 1000 ); // Лопата и кирка
    $psym[4] = array( 0, 0, 4, 40, 250, 900 ); // Динамит
    $psym[5] = array( 0, 0, 3, 30, 100, 700 ); // Кирка мешок
    $psym[6] = array( 0, 0, 2, 20, 60, 500 ); // Тачка с золотом
    $psym[7] = array( 0, 0, 1, 10, 30, 300 ); // Фонарь
    $psym[8] = array( 0, 0, 0, 5, 25, 250 ); // A
    $psym[9] = array( 0, 0, 0, 4, 20, 200 ); // K
    $psym[10] = array( 0, 0, 0, 3, 15, 150 ); // q
    $psym[11] = array( 0, 0, 0, 2, 10, 100 ); // J
    $psym[12] = array( 0, 0, 0, 1, 5, 50 ); // 10
   ...
   $casbank = winlimit( );
    ...
    while ( $am < 100000 )
    {
        ...
        for ( ; $k <= 15; ++$k )
        {
           if ( ${ "sym".$k } == 1 )
{
        $n = 0;// а вот из за чего всё и было ($n = $n + 1;) /// какой то бонус, если выпадает 3 - 4 символа // ставка умножается (см.ниже)// причём в игре об этом никак не сообщается// да и в правилах игры это никак не обозначено
}
        }
...
$mas_win = 0;
    ...
    if ( 0 < $winall )
    {
        ...
        mysql_query( "update g_set_new set g_bank=g_bank-'".$winall44."' where g_name='majormillions'" );
...
$usrbl=$user_balance-$double_win;
}

if ( $s==2 && $usrbl>=0 && $bet>0 && $lines>0)
{
    $casbank = winlimit( );
    $double_win = $_SESSION['double_win'];
    mysql_query( "update users set cash=cash-'".$double_win."' where login='{$l}'" );
    mysql_query( "update g_set_new set g_bank=g_bank+'".$double_win."' where g_name='majormillions'" );
...
echo "&riskwin=$riskwin&winall=$double_win&cash_new=$user_balance&cash=$user_balance";
...
$row = mysql_fetch_array( mysql_query( "select * from users where login='".$l."'" ) );
$user_balance = $row[3];
echo "&cash=$user_balance&cash_new=$user_balance&jpot=1243.71&txt=Welcome to casino";}
