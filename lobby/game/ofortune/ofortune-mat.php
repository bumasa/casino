...
function winlimit( )
{
    ${ "bvbvbv" } = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='ofortune'" ) );
...
if ( $freenumspin == 0 )
{
    ...
    $rowb9 = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='ofortune'" ) );
    $proc4 = $rowb9['g_proc'];
    $allbet23 = $allbet / 100 * $proc4;

    mysql_query( "update g_set_new set g_bank=g_bank+$allbet23 where g_name='ofortune'" );
    ...
    $psym[1] = array( 0, 0, 0, 0, 0, 0 ); // wild
    $psym[2] = array( 0, 0, 0, 0, 0, 0 );  // wild
    $psym[3] = array( 0, 0, 0, 50, 500, 5000 ); // Oriental Fortune
    $psym[4] = array( 0, 0, 0, 30, 300, 3000 ); // даже незнаю как это назвать.. херь какая то...
    $psym[5] = array( 0, 0, 0, 20, 200, 2000 ); //дракон
    $psym[6] = array( 0, 0, 0, 7, 70, 700 ); // рыба
    $psym[7] = array( 0, 0, 0, 6, 60, 600 ); // дом
    $psym[8] = array( 0, 0, 0, 5, 50, 500 ); // k
    $psym[9] = array( 0, 0, 0, 4, 40, 400 ); // q
    $psym[10] = array( 0, 0, 2, 4, 20, 200); // j
    $psym[11] = array( 0, 0, 1, 2, 10, 100 ); // 10
    $psym[12] = array( 0, 0, 1, 2, 10, 100 ); // 10
...
$mas_win = 0;
    ...
    if ( 0 < $winall )
    {
        ...
        mysql_query( "update g_set_new set g_bank=g_bank-'".$winall44."' where g_name='ofortune'" );
...
if ( $s==2 && $user_balance>=$betallt)
{
    ...
    mysql_query( "update g_set_new set g_bank=g_bank+'".$double_win."' where g_name='ofortune'" );
