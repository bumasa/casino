...
function winlimit( )
{
    ${ "bvbvbv" } = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='sunquest'" ) );
...
if ( $freenumspin == 0 )
{
    ...
    $rowb9 = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='sunquest'" ) );
    $proc4 = $rowb9['g_proc'];
    $allbet23 = $allbet / 100 * $proc4;

    mysql_query( "update g_set_new set g_bank=g_bank+$allbet23 where g_name='sunquest'" );
    $row_bon = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='sunquest'" ) );
    ...
    $psym[7] = array( 0, 0, 7, 15, 80, 700 ); // Фонарь
    $psym[8] = array( 0, 0, 0, 13, 60, 600 ); // A
    $psym[9] = array( 0, 0, 0, 10, 30, 300 ); // K
    $psym[10] = array( 0, 0, 0, 9, 20, 120 ); // q
    $psym[11] = array( 0, 0, 0, 7, 18, 70 ); // J
    $psym[12] = array( 0, 0, 0, 6, 13, 50 ); // 10
...
$mas_win = 0;
    ...
    if ( 0 < $winall )
    {
        ...
        mysql_query( "update g_set_new set g_bank=g_bank-'".$winall44."' where g_name='sunquest'" );
...
if ( $s==2 && $usrbl>=0 && $bet>0 && $lines>0)
{
    ...
    mysql_query( "update g_set_new set g_bank=g_bank+'".$double_win."' where g_name='sunquest'" );
