...
function winlimit( )
{
    ${ "bvbvbv" } = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='reelthunder'" ) );
...
if ( $freenumspin == 0 )
{
    ...
    $rowb9 = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='reelthunder'" ) );
    $proc4 = $rowb9['g_proc'];
    $allbet23 = $allbet / 100 * $proc4;

    mysql_query( "update g_set_new set g_bank=g_bank+$allbet23 where g_name='reelthunder'" );
    $row_bon = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='reelthunder'" ) );
    ...
    $psym[1] = array( 0, 0, 0, 5, 20, 50 ); // SCATER
    $psym[2] = array( 0, 0, 0, 0, 0, 0 );  // WILD
    $psym[3] = array( 0, 0, 12, 40, 500, 10000 ); // Лопата и кирка
    $psym[4] = array( 0, 0, 12, 25, 200, 1000 ); // Динамит
    $psym[5] = array( 0, 0, 10, 25, 150, 900 ); // Кирка мешок
    $psym[6] = array( 0, 0, 8, 15, 90, 800 ); // Тачка с золотом
    $psym[7] = array( 0, 0, 0, 13, 60, 600 ); // Фонарь
    $psym[8] = array( 0, 0, 0, 10, 30, 300 ); // A
    $psym[9] = array( 0, 0, 0, 9, 20, 120 ); // K
    $psym[10] = array( 0, 0, 0, 7, 18, 70 ); // q
    $psym[11] = array( 0, 0, 0, 6, 13, 50 ); // J
...
$mas_win = 0;
    ...
    if ( 0 < $winall )
    {
        ...
        mysql_query( "update g_set_new set g_bank=g_bank-'".$winall44."' where g_name='reelthunder'" );
...
if ( $s==2 && $usrbl>=0 && $bet>0 && $lines>0)
{
    ...
    mysql_query( "update g_set_new set g_bank=g_bank+'".$double_win."' where g_name='reelthunder'" );
