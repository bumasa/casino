...
function winlimit( )
{
    ${ "bvbvbv" } = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='gophergold'" ) );
...
if ( $freenumspin == 0 )
{
    ...
    $rowb9 = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='gophergold'" ) );
    $proc4 = $rowb9['g_proc'];
    $allbet23 = $allbet / 100 * $proc4;

    mysql_query( "update g_set_new set g_bank=g_bank+$allbet23 where g_name='gophergold'" );
    $row_bon = mysql_fetch_array( mysql_query( "select * from g_set_new where g_name='gophergold'" ) );
    ...
    $psym[1] = array( 0, 0, 0, 2, 10, 50 ); // SCATER
    $psym[2] = array( 0, 0, 0, 0, 0, 0 );  // WILD
    $psym[3] = array( 0, 0, 20, 200, 800, 5000 ); // Лопата и кирка
    $psym[4] = array( 0, 0, 5, 80, 250, 600 ); // Динамит
    $psym[5] = array( 0, 0, 5, 25, 150, 500 ); // Кирка мешок
    $psym[6] = array( 0, 0, 5, 25, 80, 300 ); // Тачка с золотом
    $psym[7] = array( 0, 0, 5, 25, 80, 250 ); // Фонарь
    $psym[8] = array( 0, 0, 0, 25, 50, 150 ); // A
    $psym[9] = array( 0, 0, 0, 25, 50, 120 ); // K
    $psym[10] = array( 0, 0, 0, 15, 50, 120 ); // q
    $psym[11] = array( 0, 0, 0, 10, 25, 100 ); // J
    $psym[12] = array( 0, 0, 0, 10, 25, 100 ); // 10
...
$mas_win = 0;
    ...
    if ( 0 < $winall )
    {
        ...
        mysql_query( "update g_set_new set g_bank=g_bank-'".$winall44."' where g_name='gophergold'" );
...
if ( $s==2 && $usrbl>=0 && $bet>0 && $lines>0)
{
    ...
    mysql_query( "update g_set_new set g_bank=g_bank+'".$double_win."' where g_name='gophergold'" );
